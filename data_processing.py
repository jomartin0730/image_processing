...
class DataProcessing:
    ...
    def remove_noise(self, path: str, algorithm: str, params: Dict[str, Any]) -> o3d.geometry.PointCloud:
        ...
        if algorithm == 'statistical':
            ...
            self.logger.info(f'Number of points after noise removal: {len(np.asarray(cl.points))}')
            return cl
            ...
            self.logger.info(f'Number of points after noise removal: {len(np.asarray(processed_img.points))}')
            return processed_img
        
        else:
            self.logger.error('Check removal algorithm')
            raise KeyError("Unknown noise removal algorithm")
 
    def project_to_2d(self, pcd: o3d.geometry.PointCloud, projection_vector: np.ndarray) -> np.ndarray:
        ...
        if projected_points.size == 0:
            self.logger.warning(f'Projected points size is not even. Current size: {projected_points}')
        else:
            self.logger.info(f'Number of projected points: {projected_points.size}')
    ...
    def create_depth_map(self, projected_points: np.ndarray, depth_map_path: str, image_size: Tuple[int, int] = (100, 100)) -> None:
        ...
        self.logger.info(f'Depth map parameters: {depth_map_image.size}, {depth_map_path}')
        self.dm.save_image(depth_map_image, depth_map_path, map_type="depth", cmap='gray')  # depth map image 저장

    def create_heat_map(self, projected_points: np.ndarray, heat_map_path: str, image_size: Tuple[int, int] = (100, 100)) -> None:
        ...
        self.logger.info(f'Heat map parameters: {heat_map_image.size}, {heat_map_path}')
